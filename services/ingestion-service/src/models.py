...
class CandidateProfile(Base):
    ...
    leadership_level_estimate: Mapped[str | None] = mapped_column(String)
    ...
    parsing_confidence: Mapped[float | None] = mapped_column(Numeric(4, 3))
    field_confidence: Mapped[dict | None] = mapped_column(JSONB)
    match_score: Mapped[float | None] = mapped_column(Numeric(4, 3))
    github_enrichment: Mapped[dict | None] = mapped_column(JSONB)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
